...
def run_with_flags(melody_rnn_sequence_generator):
  ...
  if FLAGS.primer_melody:
    ...
    tf.logging.info('primer %d', primer_sequence.tempos[0].bpm)
  elif FLAGS.primer_midi:
    primer_sequence = midi_io.midi_file_to_sequence_proto(FLAGS.primer_midi)
    tf.logging.info('primer %d', primer_sequence.tempos[0].bpm)
  else:  
    primer_melody2 = melodies_lib.MonophonicMelody()
    myRand = random.randint(48,84)
    primer_melody2.from_event_list('['+str(myRand)+']')
    primer_sequence = primer_melody2.to_sequence()
    tf.logging.info('primer rand %d', myRand)
